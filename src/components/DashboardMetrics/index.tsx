...
function GaugeChart({ percent }: { percent: number }) {
  ...
  return (
    <svg width={70} height={38} viewBox="0 0 70 38">
      <path d="M 5 35 A 30 30 0 0 1 65 35" fill="none" stroke="#374151" strokeWidth={5} strokeLinecap="round" />
  ...
  return (
    <div className="flex flex-col gap-0.5 w-full">
      ...
      <div className="flex justify-between text-[10px]">
        ...
        <span className="text-red-400">-${Math.abs(avgLoss).toFixed(0)}</span>
      </div>
    </div>
  );
}
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-6">
      ...
      <div className="bg-gray-800 rounded-lg px-4 py-3">
        <div className="flex items-center gap-1.5 mb-1">
          ...
          <DollarIcon />
          <span className="ml-auto text-xs text-gray-500">{metrics.trade_count}</span>
        </div>
        <div className="h-11 flex items-center">
          <span className={clsx(
            'text-xl font-bold',
            metrics.total_net_pnl > 0 ? 'text-green-400' : metrics.total_net_pnl < 0 ? 'text-red-400' : 'text-gray-300'
          )}>
      ...
      <div className="bg-gray-800 rounded-lg px-4 py-3">
        <div className="flex items-center gap-1.5 mb-1">
          <span className="text-xs text-gray-400">Trade win %</span>
          <TargetIcon />
        </div>
        <div className="h-11 flex items-center justify-between">
          <span className="text-xl font-bold text-gray-100">
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="flex items-center gap-2 text-[10px] -mt-1">
              <span className="text-red-400">{metrics.loss_count}</span>
              <span className="text-gray-500">{metrics.breakeven_count}</span>
              <span className="text-green-400">{metrics.win_count}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-800 rounded-lg px-4 py-3">
        <div className="flex items-center gap-1.5 mb-1">
          <span className="text-xs text-gray-400">Avg win/loss trade</span>
          <TrendIcon />
        </div>
      ...
      <div className="bg-gray-800 rounded-lg px-4 py-3">
        <div className="flex items-center gap-1.5 mb-1">
          <span className="text-xs text-gray-400">Trade expectancy</span>
          <CalculatorIcon />
        </div>
        <div className="h-11 flex items-center">
          <span className={clsx(
