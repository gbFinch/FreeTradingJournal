...
pub fn calculate_gross_pnl(direction: Direction, entry_price: f64, exit_price: f64, quantity: f64) -> f64 {
    match direction {
...
pub fn calculate_derived_fields(trade: &Trade) -> DerivedFields {
    ...
    let (gross_pnl, net_pnl, pnl_per_share) = match (trade.exit_price, trade.quantity) {
        (Some(exit), Some(qty)) => {
            let gross = calculate_gross_pnl(trade.direction, trade.entry_price, exit, qty);
...
mod tests {
    ...
    fn test_gross_pnl_long_win() {
        let pnl = calculate_gross_pnl(Direction::Long, 100.0, 110.0, 10.0);
        assert!((pnl - 100.0).abs() < 0.01);
    }

    #[test]
    fn test_gross_pnl_long_loss() {
        let pnl = calculate_gross_pnl(Direction::Long, 100.0, 90.0, 10.0);
        assert!((pnl - (-100.0)).abs() < 0.01);
    }

    #[test]
    fn test_gross_pnl_short_win() {
        let pnl = calculate_gross_pnl(Direction::Short, 100.0, 90.0, 10.0);
        assert!((pnl - 100.0).abs() < 0.01);
    }

    #[test]
    fn test_gross_pnl_short_loss() {
        let pnl = calculate_gross_pnl(Direction::Short, 100.0, 110.0, 10.0);
        assert!((pnl - (-100.0)).abs() < 0.01);
    }
